...
namespace Helion.Core.Util.Geometry.Vectors
{
    ...
    public class Vector3D
    {
        ...
        public static implicit operator Vector3D(ValueTuple<double, double, double> pair)
        {
            return new Vector3D(pair.Item1, pair.Item2, pair.Item3);
        }
        ...
        public static Vector3D operator -(in Vector3D self) => new Vector3D(-self.X, -self.Y, -self.Z);
        ...
        /// <returns>The resulting vector.</returns>
        public static Vector3D operator +(in Vector3D self, double other) => new Vector3D(self.X + other, self.Y + other, self.Z + other);
        ...
        /// <returns>The resulting vector.</returns>
        public static Vector3D operator *(in Vector3D self, double value) => new Vector3D(self.X * value, self.Y * value, self.Z * value);
        ...
        /// <returns>The resulting vector.</returns>
        public static Vector3D operator *(double value, in Vector3D self) => new Vector3D(self.X * value, self.Y * value, self.Z * value);
        ...
        /// <param name="value">The right side scalar.</param>
        /// <returns>The resulting vector.</returns>
        public static Vector3D operator /(in Vector3D self, double value) => self * (1.0 / value);
        ...
        /// <param name="self">The right side vector.</param>
        /// <returns>The resulting vector.</returns>
        public static Vector3D operator /(double value, in Vector3D self) => self * (1.0 / value);
        ...
        public static Vector3D SphereUnit(double yawRadians, double pitchRadians)
        {
            double cosYaw = Math.Cos(yawRadians);
            double sinYaw = Math.Sin(yawRadians);
            double sinPitch = Math.Sin(pitchRadians);

            return new Vector3D(sinPitch * cosYaw, sinPitch * sinYaw, cosYaw);
        }
        ...
        /// <summary>
        /// Creates a struct version of this vector.
        /// </summary>
        /// <returns>The struct version of this vector.</returns>
        public Vec3D ToStruct() => new Vec3D(X, Y, Z);

        /// <summary>
        /// Normalizes this vector. The object will be mutated as a result.
        /// Must only be done if the distance from the origin is non-zero.
        /// </summary>
        ...
        public Vector3D Floor() => new Vector3D(Math.Floor(X), Math.Floor(Y), Math.Floor(Z));

        /// <summary>
        /// Ceiling the components and returns the result.
        /// </summary>
        /// <returns>A ceiling vector.</returns>
        public Vector3D Ceil() => new Vector3D(Math.Ceiling(X), Math.Ceiling(Y), Math.Ceiling(Z));
        ...
        public Vector3D Unit() => this / Length();
        ...
        public double Component(in Vector3D onto) => Dot(onto) / onto.Length();
        ...
        public Vector3D Projection(in Vector3D onto) => Dot(onto) / onto.LengthSquared() * onto;
        ...
        public double DistanceSquared(in Vector3D other) => (this - other).LengthSquared();
        ...
        public double Distance(in Vector3D other) => (this - other).Length();
        ...
        public Vector3D Interpolate(in Vector3D end, double t) => this + (t * (end - this));
