                ...
                foreach (IArchive archive in Archives)
                {
                    ...
                    foreach (IEntry entry in archive)
                    {
                        ...
                        case "DECORATE":
                            ...
                            continue;
                        case "PLAYPAL":
                            TextureManagerNew.TrackPalette(entry);
                            continue;
                        case "PNAMES":
                            TextureDefinitionManager.TrackPNames(entry, archive);
                            continue;
                        case "TEXTURE1":
                        case "TEXTURE2":
                            TextureDefinitionManager.TrackTextureX(entry, archive);
                            continue;
                        }
                    }
                }

                // Textures should come first before others since they have no
                // dependencies and others depend on it (ex: Decorate).
                // TODO: This should be moved into the archive loop later on...
                TextureDefinitionManager.CompileAnyNewVanillaDefinitions();

                // TODO: Decorate.AttachSpriteRotationsToFrames();
