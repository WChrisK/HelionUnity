...
namespace Helion.Bsp.Geometry
{
    ...
    public class VertexAllocator : IEnumerable<BspVertex>
    {
        ...
        {
            Debug.Assert(weldingEpsilon > 0, "Cannot have a zero or negative welding epsilon");
        ...
        public Box2D Bounds()
        {
            ...
            vertices.ForEach(v =>
            {
                min = new Vec2D(Math.Min(min.X, v.X), Math.Min(min.Y, v.Y));
                max = new Vec2D(Math.Min(max.X, v.X), Math.Min(max.Y, v.Y));
            });
