...
namespace Helion.Bsp.Geometry
{
    ...
    public class BspSegment : Segment2D<BspVertex>, IGraphEdge
    {
        ...
        /// <summary>
        /// The line this is or is a subset of.
        /// </summary>
        public readonly Optional<IBspUsableLine> Line = Empty;
        ...
        /// </remarks>
        public bool OneSided => Line.Value?.OneSided ?? false;
        ...
        public BspSegment(BspVertex start, BspVertex end, int collinearIndex, IBspUsableLine line = null) :
            base(start, end)
        {
            Debug.Assert(start != end, "BSP segment shouldn't have a start and end index being the same");

            StartVertex = start;
            EndVertex = end;
            CollinearIndex = collinearIndex;
            Line = new Optional<IBspUsableLine>(line);

            start.Edges.Add(this);
            end.Edges.Add(this);

            Debug.Assert(Length() >= 0.00001, "Extremely small BSP segment detected");
        }
        ...
        {
            return $"({Start}) -> ({End}) [line={Line.Value?.Index}, oneSided={OneSided} miniseg={IsMiniseg} collinearIndex={CollinearIndex}]";
        }
    }
}
