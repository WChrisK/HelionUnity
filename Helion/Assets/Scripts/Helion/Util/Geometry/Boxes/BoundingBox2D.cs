...
namespace Helion.Util.Geometry.Boxes
{
    ...
    public class BoundingBox2D
    {
        ...
        public BoundingBox2D(in Vec2D min, in Vec2D max)
        {
            Debug.Assert(min.X <= max.X && min.Y <= max.Y, "Box3D min >= max");
        ...
        /// points.</param>
        /// <returns>A new 2D bounding box.</returns>
        public static implicit operator BoundingBox2D(ValueTuple<Vec2D, Vec2D> pair)
        {
            return new BoundingBox2D(pair.Item1, pair.Item2);
        }
        ...
        public static BoundingBox2D Combine(params BoundingBox2D[] boxes)
        {
            if (boxes.Empty())
        ...
        public static BoundingBox2D BoundSegments<TVector2D>(IEnumerable<Segment2D<TVector2D>> segments)
            where TVector2D : Vector2D
        {
            bool empty = segments.Empty();
            Debug.Assert(!empty, $"Trying to bound no segments into a {nameof(BoundingBox2D)}");

            return empty ?
                new BoundingBox2D(0, 0, 0, 0) :
                Box2D.Combine(segments.Select(s => s.Box).ToArray()).ToInstance();
        }

        /// <summary>
        /// Moves this box mutably to the new location.
        /// </summary>
        /// <param name="newCenter">The new center of the box.</param>
        public void MoveTo(in Vec2D newCenter)
        {
        ...
        public BoundingBox2D CopyOffset(in Vec2D offset) => new BoundingBox2D(Min + offset, Max + offset);
        ...
        public Seg2D GetSpanningEdge(in Vec2D position)
        {
            ...
            case 0x4: // Middle left
                return new Seg2D(TopLeft, BottomLeft);
