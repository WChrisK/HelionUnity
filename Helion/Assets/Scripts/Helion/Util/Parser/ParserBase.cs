...
using Helion.Util.Parser.Tokens;
...
namespace Helion.Util.Parser
{
    ...
    public abstract class ParserBase
    {
        ...
        private Optional<IPreprocessor> preprocessor;

        protected ParserBase(IPreprocessor tokenPreprocessor = null)
        {
            preprocessor = new Optional<IPreprocessor>(tokenPreprocessor);
        }
        ...
        public bool Parse(IEntry entry)
        {
        ...
        public bool Parse(string text)
        {
            try
            {
                ...
                if (preprocessor)
                {
                    Optional<List<Token>> tokens = preprocessor.Value.Process(Tokens);
                    if (!tokens)
                    {
                        Log.Error("Preprocessor failed, resource cannot be found");
                        return false;
                    }
        ...
        /// more tokens.
        /// </summary>
        /// <param name="text">The text that is peeked if we have tokens, or
        /// null if there are no tokens left.</param>
        /// <returns>True if there is text to peek, false if out of tokens.
        /// </returns>
        protected bool PeekCurrentText(out string text)
        {
            if (CurrentTokenIndex >= Tokens.Count)
            {
                text = null;
            ...
            text = Tokens[CurrentTokenIndex].Text;
            return true;
        }
        ...
        /// <returns>True if there is text to peek, false if out of tokens.
        /// </returns>
        protected bool PeekNextText(out string text)
        {
            ...
            text = Tokens[CurrentTokenIndex + 1].Text;
            return true;
        }
